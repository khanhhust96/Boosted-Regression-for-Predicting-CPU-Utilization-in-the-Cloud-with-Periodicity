...
def TrAdaBoostWLP(trans_S, Multi_trans_A, response_S, Multi_response_A, test, res_test, scaler, N, filename):
     # prepare trans_A
...
# load dataset 
    ...
    # rate = 0.25, rate train:test = 1:3, max_line_of_data is 4 weekdata
        ...
        train, test ,scaler, df  = read_data(filepath, rate_train_test=0.8, len_input_vector=288*12, max_line_of_data=4*7*288)
        ...
        predictWLP,predictMIN, predictMAX, predictAVG, predictBEST = TrAdaBoostWLP(trans_S, Multi_trans_A, response_S, Multi_response_A, train_test, res_test, scaler, len(LIST_PERIODS) , filename)
        res = save_final_result(filename, HORIZON, WINDOWSIZE, "WLP_"+EXPERIMENT_NAME , [predictWLP], [res_test], df)
        res = save_final_result(filename, HORIZON, WINDOWSIZE, "MIN_"+EXPERIMENT_NAME , [predictMIN], [res_test], df)
        res = save_final_result(filename, HORIZON, WINDOWSIZE, "MAX_"+EXPERIMENT_NAME , [predictMAX], [res_test], df)
        res = save_final_result(filename, HORIZON, WINDOWSIZE, "AVG_"+EXPERIMENT_NAME , [predictAVG], [res_test], df)
        res = save_final_result(filename, HORIZON, WINDOWSIZE, "BEST_"+EXPERIMENT_NAME , [predictBEST], [res_test], df)
if __name__ == "__main__":
    ...
    for horizon in [2,6,12]:
        HORIZON = horizon     
        EXPERIMENT_NAME = "Adaboost" + "_horizon" + str(horizon) 
